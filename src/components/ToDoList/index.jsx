...
function TodoList(props) {
  if (props.data.length === 0) {
    ...
    const showStar = (job) => {
      return (
        <svg id="star" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="star" class={`w-4 text-yellow-500 mr-1 ${job.completed ? `invisible` : null }`} role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
            ...
            return (
              <tr id={`row-${index}`} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={index}>
                <td className="px-6 py-4">
                  {job.priority ? showStar(job) : null}
